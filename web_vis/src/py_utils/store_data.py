...
def store_wishlist(path, wish_form):
    ...
    tmp_embedding = make_embedding(path, wish_form)
...
def make_embedding(path, wish_form):
    ## open mapping table
    ...
    ## categorical data
    wish_embedding['road/alley'] = map_table['road/alley'][wish_form['road/alley']]
    ...
    for ele in ['human_age', 'people_stream', 'house_type']:
        for key in map_table[ele]:
            if key == wish_form[ele]:
                ...
                value = 0
            wish_embedding[map_table[ele][key]] = value
